...
confirmed <- confirmedraw %>% gather(key="date", value="confirmed", -c(Country.Region, Province.State, Lat, Long)) %>% group_by(Country.Region, date) %>% summarise(confirmed=sum(confirmed))

# Refining country level death cases and aligning wide to long
deaths <- deathsraw %>% gather(key="date", value="deaths", -c(Country.Region, Province.State, Lat, Long)) %>% group_by(Country.Region, date) %>% summarise(deaths=sum(deaths))

# Refining country level recovered cases and aligning wide to long
recovered <- recoveredraw %>% gather(key="date", value="recovered", -c(Country.Region, Province.State, Lat, Long)) %>% group_by(Country.Region, date) %>% summarise(recovered=sum(recovered))
...
# USA confirmed cases
ggplot(usa, aes(x=date, y=confirmed)) + geom_bar(stat="identity", width=0.1) +
  theme_classic() +
  labs(title = "Covid-19 Confirmed Cases in USA", x= "Date", y= "Daily confirmed cases") +
  theme(plot.title = element_text(hjust = 0.5))
...
tree.worldmap_b_no_geometry <- tree(cumconfirmed ~ pop_est+economy, data = worldmap_b_no_geometry_trn)
